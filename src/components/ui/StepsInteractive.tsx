  ...
  return (
    <section className="flex flex-col items-center gap-12 lg:gap-20">
      ...
      <div className="grid lg:grid-cols-2 gap-8 lg:gap-16 items-start w-full">
        {/* Left List */}
        <div className="space-y-4 px-4 sm:px-0 order-2 lg:order-1">
                 ...
                 <div key={index} className="flex flex-col gap-2">
                   <div 
                     ...
                     className={`p-4 md:p-6 rounded-2xl cursor-pointer transition-all duration-300 border backdrop-blur-md ${
                       isActive 
                         ? 'bg-white/90 border-brand-primary/20 shadow-[0_12px_40px_rgba(11,92,255,0.12)] scale-[1.01] lg:scale-[1.03] lg:ml-6' 
                   ...
                   {/* Mobile Animation Box - Opens right below the item */}
                   {isActive && (
                     <div className="lg:hidden w-full overflow-hidden rounded-2xl mt-2 border-2 border-white bg-white/60 shadow-xl p-2 animate-in fade-in slide-in-from-top-4 duration-300">
        ...
        {/* Right Video Container - DESKTOP ONLY */}
        <div className="hidden lg:flex w-full justify-center px-4 sm:px-0 order-1 lg:order-2 sticky top-32">
